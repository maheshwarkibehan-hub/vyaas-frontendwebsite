-- ============================================
-- VYAAS AI - COMPLETE DATABASE SCHEMA (FINAL VERSION)
-- ============================================
-- Run this entire script in Supabase SQL Editor
-- This includes ALL tables and policies for the complete system

-- 1. User Notifications Table (System notifications - credit updates etc)
...
-- 2. Broadcast Notifications Table (NEW - For Admin Broadcast & User Inbox)
...
-- 6. User Sessions Table (for force logout)
...
-- 7. Update Users Table (add reward tracking columns)
...
-- 8. Update Credit Logs Table (add admin tracking)
...
-- User Notifications Policies (System notifications)
ALTER TABLE user_notifications ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Allow public insert to user_notifications" ON user_notifications;
CREATE POLICY "Allow public insert to user_notifications"
ON user_notifications FOR INSERT
WITH CHECK (true);

DROP POLICY IF EXISTS "Allow public read to user_notifications" ON user_notifications;
CREATE POLICY "Allow public read to user_notifications"
ON user_notifications FOR SELECT
USING (true);

DROP POLICY IF EXISTS "Allow public update to user_notifications" ON user_notifications;
CREATE POLICY "Allow public update to user_notifications"
ON user_notifications FOR UPDATE
USING (true);

-- Broadcast Notifications Policies (Admin Broadcast & Inbox)
